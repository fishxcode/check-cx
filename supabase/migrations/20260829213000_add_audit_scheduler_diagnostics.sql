...
ALTER TABLE public.check_configs
  ...
  ADD COLUMN IF NOT EXISTS latency_threshold_ms INTEGER NULL,
  ADD COLUMN IF NOT EXISTS next_check_at TIMESTAMPTZ NULL;

-- 调度字段范围约束（与前端输入控件一致；NULL 表示继承全局）
ALTER TABLE public.check_configs
  DROP CONSTRAINT IF EXISTS chk_check_interval_override_range,
  ADD CONSTRAINT chk_check_interval_override_range
    CHECK (check_interval_override IS NULL OR (check_interval_override BETWEEN 15 AND 600));
ALTER TABLE public.check_configs
  DROP CONSTRAINT IF EXISTS chk_latency_threshold_range,
  ADD CONSTRAINT chk_latency_threshold_range
    CHECK (latency_threshold_ms IS NULL OR (latency_threshold_ms BETWEEN 1000 AND 60000));

COMMENT ON COLUMN public.check_configs.paused_until IS '临时暂停截止时间。当前时间 < paused_until 时跳过该配置的检查。NULL 表示未暂停';
COMMENT ON COLUMN public.check_configs.pause_reason IS '暂停原因（可选）。恢复检查时清空';
COMMENT ON COLUMN public.check_configs.check_interval_override IS '自定义检查间隔（秒）。覆盖全局轮询间隔，支持 15-600 秒。NULL 表示使用全局配置';
COMMENT ON COLUMN public.check_configs.latency_threshold_ms IS '延迟阈值（毫秒）。超过此值标记为 degraded，否则为 operational。NULL 表示继承全局默认值';
...
CREATE TABLE IF NOT EXISTS public.config_audit_log (
  ...
  config_id       UUID NULL REFERENCES public.check_configs(id) ON DELETE SET NULL,
...
-- 审计日志仅由服务端 service-role 读写（绕过 RLS）。
-- 启用 RLS 且不创建任何策略：anon/authenticated 直连 PostgREST 一律被拒，
-- 防止客户端伪造 actor/action 或读取全量审计。管理端读取统一走 /api/admin/* (service-role)。
...
-- 诊断/错误分析表同样仅限服务端 service-role 访问：
-- 启用 RLS 且不建策略，阻止客户端经 PostgREST 直读配置 ID、错误详情与内部端点信息。
-- 前端一律通过 /api/admin/* 服务端路由间接读取。
ALTER TABLE public.error_recommendations ENABLE ROW LEVEL SECURITY;
...
DROP TRIGGER IF EXISTS update_error_recommendations_updated_at ON public.error_recommendations;
CREATE TRIGGER update_error_recommendations_updated_at
  BEFORE UPDATE ON public.error_recommendations
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_error_patterns_updated_at ON public.error_patterns;
